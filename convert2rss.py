...
def convert(file, metadata):
    ...
    print('<rss xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:sdx="sodix.dtd" version="2.0">', file=file)
...
def convert_resource(file, resource, publisher):
    ...
    print(f'  <sdx:language>{escape(resource["inLanguage"][0])}</sdx:language>', file=file)

    if "description" in resource and resource["description"]:
      print(f'  <description>{escape(resource["description"])}</description>', file=file)
    ...
    print(f'  <sdx:authorsWebsite>{escape(publisher["url"])}</sdx:authorsWebsite>', file=file)
    ...
    print('  <sdx:educationalLevel>Sekundarstufe I, Sekundarstufe II</sdx:educationalLevel>', file=file)
    print('  <sdx:classLevel>5-13</sdx:classLevel>', file=file)

    print(f'  <sdx:learnResourceType>{escape(get_resource_type(resource))}</sdx:learnResourceType>', file=file)
    print('  <sdx:schoolType>Realschule, Gymnasium, Mittel- / Hauptschule, Fachoberschule</sdx:schoolType>', file=file)
    ...
    if license_name:
      ...
      if version:
        print(f'  <sdx:licenseVersion>{escape(version)}</sdx:licenseVersion>', file=file)
